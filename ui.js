...
var updateMochila = () => {
  ...
  $.post("http://vrp_inventory/requestMochila", JSON.stringify({}), data => {
    ...
    $('.inventory-title').html(`
      Mochila <small>(<i><b>${data.peso.toFixed(2)}kg</b> em uso | <b>${(data.maxpeso - data.peso).toFixed(2)}kg</b> livre de <b>${data.maxpeso.toFixed(2)}kg</b> no total</i>)</small>
    `);
    if(data.inventario.length) {
      ...
      ${nameList
        .map(
          ...
          <div class="options">
            ...
            <div ${item.type == "equipar" || item.type == "recarregar" || item.index == "dinheirosujo" || item.index == "cocaina" || item.index == "metanfetamina" || item.index == "maconha" ? 'disabled style="opacity: 0.3;"' : ''} class="option" data-event="drop">Dropar</div>
          </div>
          ...
          <div class="row">
            <span class="amount">${formatarNumero(item.amount)} (${(item.peso * item.amount).toFixed(2)}kg)</span>
          </div>
          <div class="row">
      ...
      $(".row.objects").html("Sua mochila está vazia.");
    }
  });
